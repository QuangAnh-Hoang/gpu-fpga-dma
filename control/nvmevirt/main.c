...
extern unsigned long nvmev_io_lat_ns;
...
static bool nvmev_proc_dbs(void)
{
	...
	bool fast_path = (nvmev_io_lat_ns != 0);
#endif

	// Admin queue (always handled by dispatcher)
	...
	// Submission queues (workers handle these in fast-path mode)
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
	if (fast_path)
		goto skip_sq;
#endif
...
skip_sq:
#endif
	// Completion queues (always handled by dispatcher)
...
/*
 * /proc/nvmev/io_lat_ns — fast-path control and I/O latency override.
 *
 * When non-zero (default: 1), IO workers poll doorbells directly and
 * handle data copy + CQ fill inline, bypassing FTL and the dispatcher
 * pipeline. Writing 0 restores the normal FTL-simulated path.
 */

static int __io_lat_ns_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%lu\n", nvmev_io_lat_ns);
	return 0;
}

static int __io_lat_ns_open(struct inode *inode, struct file *file)
{
	return single_open(file, __io_lat_ns_show, NULL);
}

static ssize_t __io_lat_ns_write(struct file *file, const char __user *buf,
				 size_t count, loff_t *ppos)
{
	unsigned long val;
	int rc;

	rc = kstrtoul_from_user(buf, count, 0, &val);
	if (rc)
		return rc;

	WRITE_ONCE(nvmev_io_lat_ns, val);
	NVMEV_INFO("I/O latency override set to %lu ns%s\n",
		   val, val ? "" : " (FTL simulation restored)");
	return count;
}

#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 0, 0)
static const struct proc_ops io_lat_ns_fops = {
	.proc_open = __io_lat_ns_open,
	.proc_write = __io_lat_ns_write,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
#else
static const struct file_operations io_lat_ns_fops = {
	.open = __io_lat_ns_open,
	.write = __io_lat_ns_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
	if (nvmev_gpu_mem_init() < 0) {
		NVMEV_ERROR("GPU Direct I/O init failed\n");
		goto ret_err;
	}
	if (nvmev_gpu_procfs_init(nvmev_vdev->proc_root) < 0) {
		NVMEV_ERROR("GPU procfs init failed\n");
		nvmev_gpu_mem_exit();
		goto ret_err;
	}
#endif
