...
/*
 * Runtime-configurable I/O latency override and fast-path control.
 *
 * When non-zero, enables the fast-path: IO workers poll doorbells
 * directly and perform data copy + CQ fill inline, bypassing the
 * FTL and the dispatcher→worker pipeline entirely.
 *
 * Controllable via /proc/nvmev/io_lat_ns.
 *
 * Values:
 *   0       = disabled, use normal FTL-simulated path
 *   1       = fast-path with instant completion (default)
 *   1000    = fast-path with 1 µs fixed latency per command
 *   10000   = fast-path with 10 µs fixed latency per command
 */
unsigned long nvmev_io_lat_ns = 1;
...
/* ========================================================================
 * GPU-aware PRP address mapping helpers
 *
 * These functions handle three types of physical addresses in PRPs:
 *   1. Host RAM (pfn_valid) → kmap_atomic_pfn
 *   2. GPU BAR1 (registered) → persistent ioremap_wc via lookup table
 *   3. Other MMIO            → memremap
 *
 * For GPU mappings, rcu_read_lock must be held around the entire use of
 * the returned pointer to prevent the mapping from being freed.
 * ======================================================================== */

#define NVMEV_MAP_HOST		0
#define NVMEV_MAP_GPU		1
#define NVMEV_MAP_MMIO		2
#define NVMEV_MAP_GPU_AUTO	3

static inline void *__nvmev_map_prp(u64 paddr, int *map_type)
{
	if (pfn_valid(paddr >> PAGE_SHIFT)) {
		*map_type = NVMEV_MAP_HOST;
		return kmap_atomic_pfn(PRP_PFN(paddr));
	}

#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
	{
		void __iomem *gpu_ptr;

		rcu_read_lock();
		gpu_ptr = nvmev_gpu_kmap(paddr);
		if (gpu_ptr) {
			*map_type = NVMEV_MAP_GPU;
			return (void *)gpu_ptr;
		}
		rcu_read_unlock();

		gpu_ptr = nvmev_gpu_auto_map(paddr);
		if (gpu_ptr) {
			*map_type = NVMEV_MAP_GPU_AUTO;
			return (void *)gpu_ptr;
		}
	}
#endif

	*map_type = NVMEV_MAP_MMIO;
	return memremap(paddr, PAGE_SIZE, MEMREMAP_WT);
}

static inline void __nvmev_unmap_prp(void *vaddr, int map_type)
{
	switch (map_type) {
	case NVMEV_MAP_HOST:
		kunmap_atomic(vaddr);
	...
	case NVMEV_MAP_GPU:
		rcu_read_unlock();
		break;
	case NVMEV_MAP_GPU_AUTO:
		break;
#endif
	}
}

static inline void __nvmev_io_copy(void *prp_vaddr, int map_type,
				   size_t mem_offs, void *storage,
				   size_t io_size, bool to_storage)
{
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
	if (map_type == NVMEV_MAP_GPU || map_type == NVMEV_MAP_GPU_AUTO) {
		void __iomem *io_ptr = (void __iomem *)prp_vaddr + mem_offs;

		if (to_storage)
			nvmev_memcpy_from_gpu(storage, io_ptr, io_size);
		else
			nvmev_memcpy_to_gpu(io_ptr, storage, io_size);
		return;
	}
#endif
	if (to_storage)
		memcpy(storage, prp_vaddr + mem_offs, io_size);
	else
		memcpy(prp_vaddr + mem_offs, storage, io_size);
}
...
static unsigned int __do_perform_io(int sqid, int sq_entry)
{
	...
	int paddr_list_map_type = NVMEV_MAP_HOST;
	bool to_storage;

	offset = __cmd_io_offset(cmd);
	length = __cmd_io_size(cmd);
	remaining = length;
	to_storage = (cmd->opcode == nvme_cmd_write ||
	...
	while (remaining) {
		...
		if (prp_offs == 1) {
			...
			if (remaining > PAGE_SIZE) {
				paddr_list = __nvmev_map_prp(paddr,
							&paddr_list_map_type);
				paddr_list = (u64 *)((u8 *)paddr_list +
		...
		vaddr = __nvmev_map_prp(paddr, &vaddr_map_type);
		...
		__nvmev_io_copy(vaddr, vaddr_map_type, mem_offs,
				nvmev_vdev->ns[nsid].mapped + offset,
				io_size, to_storage);

		__nvmev_unmap_prp(vaddr, vaddr_map_type);
	...
	if (paddr_list) {
		paddr_list = (u64 *)((u8 *)paddr_list -
				(cmd->prp2 & PAGE_OFFSET_MASK));
		__nvmev_unmap_prp(paddr_list, paddr_list_map_type);
	}
...
/*
 * ISE_OPC_MASK_READ: Near-storage masked read command.
 *
 * Reads two 4KB pages from emulated storage into a per-worker staging
 * buffer (2nd read overwrites 1st), then streams the staging buffer
 * to the GPU PRP1 address.
 *
 * Command field mapping:
 *   CDW10 = byte offset of page 0 into namespace (4KB-aligned)
 *   CDW11 = byte offset of page 1 into namespace (4KB-aligned)
 *   PRP1  = GPU destination physical address (4KB-aligned)
 *   result0 = number of bytes written to GPU
 */
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
static bool __ise_mask_read(int sqid, int sq_entry_idx, size_t *io_size)
{
	struct nvmev_submission_queue *sq = nvmev_vdev->sqes[sqid];
	struct nvmev_completion_queue *cq = nvmev_vdev->cqes[sq->cqid];
	struct nvme_common_command *cmd = &sq_entry(sq_entry_idx).common;
	unsigned int command_id = cmd->command_id;
	uint32_t nsid = cmd->nsid - 1;
	struct nvmev_ns *ns = &nvmev_vdev->ns[nsid];
	unsigned int io_worker_id = __get_io_worker(sqid);
	struct nvmev_io_worker *worker = &nvmev_vdev->io_workers[io_worker_id];
	void *staging = worker->ise_staging;
	uint64_t offset0 = cmd->cdw10[0]; /* CDW10: byte offset page 0 */
	uint64_t offset1 = cmd->cdw10[1]; /* CDW11: byte offset page 1 */
	int map_type;
	void *gpu_dst;
	int cq_head;

	BUG_ON(!staging);
	WARN_ON(offset0 + PAGE_SIZE > ns->size);
	WARN_ON(offset1 + PAGE_SIZE > ns->size);

	memcpy(staging, ns->mapped + offset0, PAGE_SIZE);
	memcpy(staging, ns->mapped + offset1, PAGE_SIZE);

	gpu_dst = __nvmev_map_prp(cmd->prp1, &map_type);
	kernel_fpu_begin();
	nvmev_memcpy_to_gpu(gpu_dst, staging, PAGE_SIZE);
	kernel_fpu_end();
	__nvmev_unmap_prp(gpu_dst, map_type);
	...
	*io_size = PAGE_SIZE;
	...
	if (cq->is_gpu) {
		struct nvme_completion local_cqe = {
			.result0 = PAGE_SIZE,
			.command_id = command_id,
			.sq_id = sqid,
			.sq_head = sq_entry_idx,
			.status = cq->phase | (NVME_SC_SUCCESS << 1),
		};

		kernel_fpu_begin();
		nvmev_memcpy_to_gpu((void __iomem *)&cq_entry(cq_head),
				    &local_cqe, sizeof(local_cqe));
		kernel_fpu_end();
	} else {
		...
		cqe->sq_id = sqid;
		cqe->sq_head = sq_entry_idx;
		cqe->status = cq->phase | (NVME_SC_SUCCESS << 1);
		cqe->result0 = PAGE_SIZE;
		cqe->result1 = 0;
	}
...
/*
 * Dispatcher fast-path: inline data copy + CQ fill on the dispatcher CPU.
 *
 * Bypasses the entire IO worker pipeline (FTL, work queue, cross-CPU handoff,
 * sorted insertion, reclamation) when io_lat_ns is set. The dispatcher
 * handles the complete I/O synchronously:
 *   doorbell → parse cmd → memcpy → write CQ → done
 *
 * This eliminates ~3-12 µs of per-command overhead from the normal path.
 */
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
static bool __fast_path_io(int sqid, int sq_entry_idx)
{
	struct nvmev_submission_queue *sq = nvmev_vdev->sqes[sqid];
	struct nvmev_completion_queue *cq = nvmev_vdev->cqes[sq->cqid];
	struct nvme_rw_command *cmd = &sq_entry(sq_entry_idx).rw;
	unsigned int command_id = sq_entry(sq_entry_idx).common.command_id;
	unsigned long long target = local_clock() + nvmev_io_lat_ns;
	int cq_head;

	kernel_fpu_begin();
	__do_perform_io(sqid, sq_entry_idx);
	kernel_fpu_end();

	/* Spin until the configured latency budget has elapsed.
	 * The memcpy time above counts toward the budget.
	 * Skipped when io_lat_ns <= 1 (instant completion). */
	if (nvmev_io_lat_ns > 1)
		while (local_clock() < target)
			cpu_relax();
...
static size_t __nvmev_proc_io(int sqid, int sq_entry, size_t *io_size)
{
	...
	if (unlikely(nvmev_io_lat_ns)) {
		if (cmd->common.opcode == nvme_cmd_read ||
			...
			*io_size = __cmd_io_size(&sq_entry(sq_entry).rw);
			return __fast_path_io(sqid, sq_entry);
		}
		if (cmd->common.opcode == ISE_OPC_MASK_READ)
			return __ise_mask_read(sqid, sq_entry, io_size);
	}
#endif

	ns = &nvmev_vdev->ns[nsid];

	req = (struct nvmev_request){
		.cmd = cmd,
		.sq_id = sqid,
		.nsecs_start = nsecs_start,
	};
	ret = (struct nvmev_result){
	...
	*io_size = __cmd_io_size(&sq_entry(sq_entry).rw);
	...
	if (unlikely(nvmev_io_lat_ns))
		ret.nsecs_target = nsecs_start + nvmev_io_lat_ns;
 ...
 * performing the complete I/O inline (memcpy + CQ write) without any
 * cross-CPU handoff. This distributes work across all worker CPUs.
 */
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
static void __worker_poll_fast_path(struct nvmev_io_worker *worker)
{
	int qid, dbs_idx, new_db, old_db;

	for (qid = 1; qid <= nvmev_vdev->nr_sq; qid++) {
		struct nvmev_submission_queue *sq_check = nvmev_vdev->sqes[qid];
		if (sq_check == NULL)
			continue;
		if (__get_io_worker(qid) != worker->id)
			continue;
...
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
		if (unlikely(nvmev_io_lat_ns)) {
			__worker_poll_fast_path(worker);
			cond_resched();
			continue;
		}
#endif
...
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
		worker->ise_staging = kmalloc(PAGE_SIZE, GFP_KERNEL);
		if (!worker->ise_staging)
			NVMEV_ERROR("Failed to allocate ISE staging buffer for worker %d\n", worker_id);
#endif
