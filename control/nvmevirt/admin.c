...
static void __nvmev_admin_create_cq(int eid)
{
	...
	} else if (nvmev_is_gpu_addr(cmd->prp1)) {
		cq->mapped = NULL;
		cq->is_gpu = true;
		rcu_read_lock();
		for (i = 0; i < num_pages; i++) {
			u64 page_addr = (cmd->prp1 & PAGE_MASK) + i * PAGE_SIZE;
			void __iomem *gpu_ptr = nvmev_gpu_kmap(page_addr);

			cq->cq[i] = (struct nvme_completion __iomem *)gpu_ptr;
		}
		rcu_read_unlock();
		NVMEV_INFO("CQ %d created in GPU memory (prp1=0x%llx)\n",
			   cq->qid, cmd->prp1);
#endif
	} else {
...
static void __nvmev_admin_create_sq(int eid)
{
	...
	if (pfn_valid(cmd->prp1 >> PAGE_SHIFT)) {
		...
		for (i = 0; i < num_pages; i++) {
			...
			sq->sq[i] = (struct nvme_command __iomem *)gpu_ptr;
		}
		rcu_read_unlock();
		NVMEV_INFO("SQ %d created in GPU memory (prp1=0x%llx)\n",
			   sq->qid, cmd->prp1);
#endif
	} else {
