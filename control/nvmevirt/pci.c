...
bool nvmev_proc_bars(void)
{
	...
	if (old_bar->asq != bar->u_asq) {
		...
		queue->sq_is_gpu = false;
		if (pfn_valid(nvmev_vdev->bar->u_asq >> PAGE_SHIFT)) {
			for (i = 0; i < num_pages; i++)
				queue->nvme_sq[i] = page_address(
					pfn_to_page(nvmev_vdev->bar->u_asq >> PAGE_SHIFT) + i);
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
		} else if (nvmev_is_gpu_addr(nvmev_vdev->bar->u_asq)) {
			queue->sq_is_gpu = true;
			rcu_read_lock();
			for (i = 0; i < num_pages; i++) {
				u64 pa = (nvmev_vdev->bar->u_asq & PAGE_MASK) + i * PAGE_SIZE;
				queue->nvme_sq[i] = (void *)nvmev_gpu_kmap(pa);
			}
			rcu_read_unlock();
			NVMEV_INFO("Admin SQ mapped from GPU memory\n");
#endif
		} else {
			NVMEV_ERROR("ASQ address 0x%llx is not in valid memory\n",
				    nvmev_vdev->bar->u_asq);
		}
	...
	if (old_bar->acq != bar->u_acq) {
		...
		queue->cq_is_gpu = false;
		if (pfn_valid(nvmev_vdev->bar->u_acq >> PAGE_SHIFT)) {
			for (i = 0; i < num_pages; i++)
				queue->nvme_cq[i] = page_address(
					pfn_to_page(nvmev_vdev->bar->u_acq >> PAGE_SHIFT) + i);
#ifdef CONFIG_NVMEVIRT_GPU_DIRECT
		} else if (nvmev_is_gpu_addr(nvmev_vdev->bar->u_acq)) {
			queue->cq_is_gpu = true;
			rcu_read_lock();
			for (i = 0; i < num_pages; i++) {
				u64 pa = (nvmev_vdev->bar->u_acq & PAGE_MASK) + i * PAGE_SIZE;
				queue->nvme_cq[i] = (void *)nvmev_gpu_kmap(pa);
			}
			rcu_read_unlock();
			NVMEV_INFO("Admin CQ mapped from GPU memory\n");
#endif
		} else {
			NVMEV_ERROR("ACQ address 0x%llx is not in valid memory\n",
				    nvmev_vdev->bar->u_acq);
		}
